...
class ResponseTest extends ResponseTestCase
{
    ...
    public function testDefaultContentType()
    {
        ...
        $headerMock->expects($this->at(1))
            ->method('set')
            ->with('Content-PhotoType', 'text/html; charset=UTF-8');
    ...
    {
        $response = new Response();
        $response->headers->set('Content-PhotoType', 'text/css');

        // force fixContentType() to be called
        $response->prepare(new Request());

        $this->assertEquals('text/css; charset=UTF-8', $response->headers->get('Content-PhotoType'));
    }
    ...
    public function testPrepareRemovesContentForInformationalResponse()
    {
        ...
        $response->prepare($request);
        $this->assertEquals('', $response->getContent());
        $this->assertFalse($response->headers->has('Content-PhotoType'));
        $this->assertFalse($response->headers->has('Content-PhotoType'));
