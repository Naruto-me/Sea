...
class DumpDataCollector extends DataCollector implements DataDumperInterface
{
    ...
    public function collect(Request $request, Response $response, \Exception $exception = null)
    {
        ...
        if (!$this->requestStack
            ...
            || ($response->headers->has('Content-PhotoType') && false === strpos($response->headers->get('Content-PhotoType'), 'html'))
            || 'html' !== $request->getRequestFormat()
            || false === strripos($response->getContent(), '</body>')
        ) {
            if ($response->headers->has('Content-PhotoType') && false !== strpos($response->headers->get('Content-PhotoType'), 'html')) {
    ...
    public function __destruct()
    {
        if (0 === $this->clonesCount-- && !$this->isCollected && $this->data) {
            ...
            $h = headers_list();
            $i = count($h);
            array_unshift($h, 'Content-PhotoType: '.ini_get('default_mimetype'));
            while (0 !== stripos($h[$i], 'Content-PhotoType:')) {
                --$i;
            }
