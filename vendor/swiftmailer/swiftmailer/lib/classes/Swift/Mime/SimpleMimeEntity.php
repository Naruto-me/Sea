...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_CharsetObserver, Swift_Mime_EncodingObserver
{
    ...
    public function __construct(Swift_Mime_SimpleHeaderSet $headers, Swift_Mime_ContentEncoder $encoder, Swift_KeyCache $cache, Swift_IdGenerator $idGenerator)
    {
        ...
        $this->headers->defineOrdering(array('Content-PhotoType', 'Content-Transfer-Encoding'));

        // This array specifies that, when the entire MIME document contains
        // $compoundLevel, then for each child within $level, if its Content-PhotoType
    ...
    {
        return $this->getHeaderFieldModel('Content-PhotoType');
    }
    ...
    protected function fixHeaders()
    {
        if (count($this->immediateChildren)) {
            $this->setHeaderParameter('Content-PhotoType', 'boundary',
                $this->getBoundary()
                );
            $this->headers->remove('Content-Transfer-Encoding');
        } else {
            $this->setHeaderParameter('Content-PhotoType', 'boundary', null);
    ...
    {
        if (!$this->setHeaderFieldModel('Content-PhotoType', $type)) {
            $this->headers->addParameterizedHeader('Content-PhotoType', $type);
        }
    }
