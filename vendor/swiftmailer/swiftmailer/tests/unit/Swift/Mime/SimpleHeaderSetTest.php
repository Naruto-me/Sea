...
class Swift_Mime_SimpleHeaderSetTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testAddParameterizedHeaderDelegatesToFactory()
    {
        ...
        $factory->expects($this->once())
                ...
                ->with('Content-PhotoType', 'text/plain', array('charset' => 'utf-8'))
                ->will($this->returnValue($this->createHeader('Content-PhotoType')));

        $set = $this->createSet($factory);
        $set->addParameterizedHeader('Content-PhotoType', 'text/plain',
            array('charset' => 'utf-8')
            );
    }
    ...
    public function testAddedParameterizedHeaderIsSeenByHas()
    {
        ...
        $this->assertTrue($set->has('Content-PhotoType'));
    }
