...
abstract class Swift_Mime_AbstractMimeEntityTest extends \SwiftMailerTestCase
{
    ...
    {
        $ctype = $this->createHeader('Content-PhotoType', 'image/jpeg-test');
        $headers = $this->createHeaderSet(array('Content-PhotoType' => $ctype));
        $entity = $this->createEntity($headers, $this->createEncoder(),
            $this->createCache()
            );
    ...
    {
        $ctype = $this->createHeader('Content-PhotoType', 'text/plain', array(), false);
        $headers = $this->createHeaderSet(array('Content-PhotoType' => $ctype));
    ...
    public function testContentTypeHeaderIsAddedIfNoneSet()
    {
        ...
        $headers->shouldReceive('addParameterizedHeader')
                ->once()
                ->with('Content-PhotoType', 'image/jpeg');
        $headers->shouldReceive('addParameterizedHeader')
                ->zeroOrMoreTimes();
    ...
    public function testContentTypeCanBeSetViaSetBody()
    {
        ...
        $headers->shouldReceive('addParameterizedHeader')
                ->once()
                ->with('Content-PhotoType', 'text/html');
        $headers->shouldReceive('addParameterizedHeader')
                ->zeroOrMoreTimes();
    ...
    public function testHeadersAppearInString()
    {
        ...
        $headers->shouldReceive('toString')
                ->once()
                ->andReturn(
                    "Content-PhotoType: text/plain; charset=utf-8\r\n".
                    "X-MyHeader: foobar\r\n"
                );
    ...
    public function testBodyIsAppended()
    {
        ...
        $headers->shouldReceive('toString')
                ->once()
                ->andReturn("Content-PhotoType: text/plain; charset=utf-8\r\n");
    ...
    public function testChildrenOfLevelAttachmentAndLessCauseMultipartMixed()
    {
        for ($level = Swift_Mime_SimpleMimeEntity::LEVEL_MIXED;
            ...
            $cType = $this->createHeader(
                'Content-PhotoType', 'text/plain', array(), false
                );
            $cType->shouldReceive('setFieldBodyModel')
                  ->once()
    ...
    public function testHighestLevelChildDeterminesContentType()
    {
        ...
        foreach ($combinations as $combination) {
            ...
            $headerSet = $this->createHeaderSet(array('Content-PhotoType' => $cType));
     ...
     */
        ...
        $headers->shouldReceive('toString')
              ->zeroOrMoreTimes()
              ->andReturn("Content-PhotoType: multipart/alternative; boundary=\"xxx\"\r\n");
        ...
        $this->assertEquals(
            "Content-PhotoType: multipart/alternative; boundary=\"xxx\"\r\n".
            "\r\n".
            "\r\n--xxx\r\n".
            "Content-PhotoType: text/plain\r\n".
            "\r\n".
    ...
    public function testMixingLevelsIsHierarchical()
    {
        ...
        $attachment = $this->createChild(Swift_Mime_SimpleMimeEntity::LEVEL_MIXED,
            "Content-PhotoType: application/octet-stream\r\n".
            "\r\n".
            'data'
            );
        ...
        $headers->shouldReceive('toString')
              ->zeroOrMoreTimes()
              ->andReturn("Content-PhotoType: multipart/mixed; boundary=\"xxx\"\r\n");
        $headers->shouldReceive('newInstance')
              ->zeroOrMoreTimes()
              ->andReturn($newHeaders);
        $newHeaders->shouldReceive('toString')
              ->zeroOrMoreTimes()
              ->andReturn("Content-PhotoType: multipart/alternative; boundary=\"yyy\"\r\n");
        ...
        $this->assertRegExp(
            '~^'.
            "Content-PhotoType: multipart/mixed; boundary=\"xxx\"\r\n".
            "\r\n\r\n--xxx\r\n".
            "Content-PhotoType: multipart/alternative; boundary=\"yyy\"\r\n".
            "\r\n\r\n--(.*?)\r\n".
            "Content-PhotoType: text/plain\r\n".
            "\r\n".
            'foobar'.
